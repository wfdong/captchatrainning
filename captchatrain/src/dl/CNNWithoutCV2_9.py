...
数据文件这样分部，train-0,train-1,...train-11, test
@author: rogers
'''
"""
用Imagenet的方法，多个卷积层
...
def saveParams(epoch_counter, params):
    ...
    save_file = open('../data/params-9-' + str(epoch_counter), 'wb')
    print(len(params))
    print(type(params))
    for p in params:
...
def loadTrainDataWithIndex(index):
    save_file = open('../data/db14/train-' + str(index), 'rb')
    imageLabels = cPickle.load(save_file)
    save_file.close()
    allImg = []
    allLabels = []
    for item in imageLabels:
        allImg.append(item[0])
        allLabels.append(item[1])
    imgs = numpy.asarray(allImg)
    labels = numpy.asarray(allLabels)
    print("Read train data train-%d, Train size %s" % (index, str(imgs.shape)))
    #print("Read train data train-" + str(index))
    #print("Train size:")
    #print(imgs.shape)
    #print(labels.shape)
    #train_set_x, train_set_y = shared_dataset(imgs, labels)
    return (imgs, labels)

def loadTest():
    save_file = open('../data/db14/test', 'rb')
...
def loadValid():
    valids = []
    for i in range(2):
        save_file = open('../data/db14/train-' + str(i + 1), 'rb')
        allImageLabels = cPickle.load(save_file)
        random.shuffle(allImageLabels)
        valids.extend(allImageLabels[0:1000])
        save_file.close()
    ...
    valid_set_x, valid_set_y = shared_dataset(imgs, labels)
