...
这是whole img的版本，input img = 130 * 53
@author: rogers
'''
"""
用Imagenet的方法，多个卷积层
...
layer2:(1,12,12) * (192,3,3) -> (192,5,5)
'''
def evaluate_lenet5(learning_rate=0.1, n_epochs=120,
    ...
    ishape = (50, 100)  # this is the size of MNIST images
    ...
    layer0_input = x.reshape((batch_size, 1, 100, 50))
    ...
    layer0 = LeNetConvPoolLayer(rng, input=layer0_input,
            image_shape=(batch_size, 1, 100, 50),
    ...
    layer1 = LeNetConvPoolLayer(rng, input=layer0.output,
            image_shape=(batch_size, nkerns[0], 48, 23),
    ...
    layer1_4 = LeNetConvPoolLayer(rng, input=layer1_3.output,
            image_shape=(batch_size, nkerns[2], 21, 8),
    ...
    layer2 = HiddenLayer(rng, input=layer2_input, n_in=nkerns[3] * 9 * 3,
                         n_out=1920, activation=T.tanh)

    # classify the values of the fully-connected sigmoidal layer
    layer3 = LogisticRegression(input=layer2.output, n_in=1920, n_out=58)
        ...
        for minibatch_index in xrange(n_train_batches):
            ...
            if (iter + 1) % validation_frequency == 0:
                ...
                if this_validation_loss < best_validation_loss:
                    ...
                    test_losses = [test_model(i) for i in xrange(n_test_batches)]
                    test_score = numpy.mean(test_losses)
                    print(('     best epoch %i, minibatch %i/%i, test error of best '
                           'model %f %%') %
                          (epoch, minibatch_index + 1, n_train_batches,
                           test_score * 100.))
                    best_params = params
...
def loadData():
    ...
    save_file = open('../data/dbWhole', 'rb')
