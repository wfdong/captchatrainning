...
def split2Whole(imgPath):
    ...
    for i in range(1):
        crop_img = img[1:51, i1[0]:i1[1]]
        splitChar = "_"
        if ((ord)(labels[i])-ord('a')) >= 0:
             splitChar = "__"
        fileName = Constants.IMG_DIR_TENCENT_WHOLE + labels[i] + splitChar + str(indexDict[(ord)(labels[i])-ord('A')]) + ".jpg"
        if TEST:
            fileName = Constants.IMG_DIR_TENCENT_TEST_WHOLE + labels[i] + splitChar + str(indexDict[(ord)(labels[i])-ord('A')]) + ".jpg"
        #print(fileName)
...
def wrapAffine2(imgPath):
    ...
    for i in range(len(pts2All)):
        ...
        dst = cv2.warpAffine(~img,M,(cols,rows))
        fileName = Constants.IMG_DIR_TENCENT_WHOLE + img_name[:img_name.index(".jpg")] + "_" + str(indexDict[(ord)(label)-ord('A')]) + ".jpg"
        if TEST:
            fileName = Constants.IMG_DIR_TENCENT_TEST_WHOLE + img_name[:img_name.index(".jpg")] + "_" + str(indexDict[(ord)(label)-ord('A')]) + ".jpg"
...
if __name__ == "__main__":
    ...
    initDict()
    #process train data
    Util.readAllImg(Constants.IMG_DIR_TENCENT_NEW_TRAIN, split2Whole)
    #initDict()
    Util.readAllImg(Constants.IMG_DIR_TENCENT_WHOLE, wrapAffine2)
    #Util.readAllImg(Constants.IMG_DIR_TENCENT_SPLIT_ALL, resizeAllImg)
    
    TEST = True
    initDict()
    #process test data
    Util.readAllImg(Constants.IMG_DIR_TENCENT_TEST, split2Whole)
    initDict()
    Util.readAllImg(Constants.IMG_DIR_TENCENT_TEST_WHOLE, wrapAffine2)
